...
> The `fiscale` key is retained for stored-data compatibility; historical
> Italian tax examples do not change Specter's APAC/common-law focus.
...
Specter ships JSON workflow presets across professional domains. Legal
content includes SG/common-law review and the intentional English EU AI Act
workflow. Presets live in [`config/workflow-presets/`](../config/workflow-presets/),
are loaded by [`src/presets/workflow.rs`](../src/presets/workflow.rs), and are
merged with user-created workflows in the backend API response.

In the UI:

- Built-ins have an "Built-in" badge.
...
- You **can duplicate** it (right-click → Duplicate). The duplicate
  ...
  for variants ("NDA Review — Singapore jurisdiction requirements").
...
- **No DB seed for built-ins.** They live in the JSON preset registry, not
...
- **Practice list is shipped.** Adding a permanent new practice area
  ...
  [`WorkflowModal.svelte`](../frontend/src/lib/components/workflow/WorkflowModal.svelte)
...
The persistent domain keys (`legal`, `medical`, `finance`,
`fiscale`, `real_estate`, `hr`, `insurance`, `ip`, `compliance`,
`gdpr`, `pa`, `others`) cover professional verticals — the
`fiscale` (legacy tax-domain identifier) vertical was added in v0.7.0
as a worked example of the full add-a-domain flow (see
`docs/piano_settore_fiscale.md`, a historical plan removed from this fork). To add another (e.g. `architecture`,
`journalism`):
...
2. Edit
   ...
   array constant.
3. Add an English display label under `Domains.values.<new_id>` in
...
Update the `PRACTICES` list in
[`WorkflowModal.svelte`](../frontend/src/lib/components/workflow/WorkflowModal.svelte)
and the corresponding list in
[`WorkflowEditor.svelte`](../frontend/src/lib/components/workflow/WorkflowEditor.svelte).
Add an English display label under `Workflows.practiceLabels` in
[`frontend/locales/en.json`](../frontend/locales/en.json), then rebuild the
frontend (or use development HMR). Preserve existing stored practice values.
...
the column-preset controls in the tabular editor. Each file shape:
...
| What | Where |
|---|---|
| Workflow type definition | [`frontend/src/lib/types/workflow.ts`](../frontend/src/lib/types/workflow.ts) |
| Column type + formats | [`frontend/src/lib/types/workflow.ts`](../frontend/src/lib/types/workflow.ts) |
| Built-in workflows | [`src/presets/workflow.rs`](../src/presets/workflow.rs) |
| Practice areas dropdown | [`frontend/src/lib/components/workflow/WorkflowModal.svelte`](../frontend/src/lib/components/workflow/WorkflowModal.svelte) |
| Workflow editor UI | [`frontend/src/lib/components/workflow/WorkflowEditor.svelte`](../frontend/src/lib/components/workflow/WorkflowEditor.svelte) |
| Tabular review UI | [`frontend/src/lib/components/tabular/`](../frontend/src/lib/components/tabular/) |
...
| i18n strings | [`frontend/locales/en.json`](../frontend/locales/en.json) under `Workflows`, `TabularReviews`, `Assistant`, `WorkflowColumns` |
